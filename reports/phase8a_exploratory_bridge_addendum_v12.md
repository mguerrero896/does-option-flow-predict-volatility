# Phase 8A exploratory bridge addendum v12
...
**Current authority:** This addendum supersedes v11 for the Phase 8A interpretation. The
v1–v11 paths remain frozen as audit evidence.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| D | Gamma GLM | 0.002615 | 0.013975 | 0.19 | 0.013891 | 0.003106 | 1.32 | 0.71 |
| D | LightGBM | 0.003000 | 0.014645 | 0.20 | 0.003803 | 0.000850 | 5.04 | 2.42 |
| V | Gamma GLM | 0.003386 | 0.007664 | 0.44 | 0.007378 | 0.001650 | 1.36 | 1.69 |
| V | LightGBM | 0.006203 | 0.018778 | 0.33 | 0.008672 | 0.001939 | 2.83 | 3.32 |
...
reductions are 2.42-fold for D/LightGBM, 1.69-fold for V/Gamma GLM and 3.32-fold for
...
`739c6044e5469192a758b7de12b2c98f5a52d679efb5d61f9560fa1cf74ae078`.
...
This current reference is the RP2-v3 B1 spot-cutoff remediation run
`rp2-v3-20260831-b1-spot-cutoff-remediation`, measured at code commit
`b70c54ba14fdda2197efd6bcf0aa676c4ba3d4f1`. Bar roles are derived from partition dates;
B0 predictors, B1 option rows and B1 underlying spot share the registered 120-second
cutoff; and B1 uses the same XNYS expiry-close calendar as B2. The Phase 8 cube and
published Phase 8 cells are unchanged; only the independently measured D/V comparison is
superseded.
...
The 128-file audit/replay producer closure is independently frozen before this artifact
in `artifacts/phase8_bridge/dispersion_audit_producer_freeze_v6.json`. It contains the
audit builder, Phase 8 evaluator, Block 12 producer, complete `src/mds650` tree and
`uv.lock`; its aggregate SHA-256 is
`7e6c90d9bdbab6ec05c977c63fae2191103901195d42bb50095ea28307b4bbf4`.
The registered freeze file SHA-256 is
`cabfea8715846b27373d3333ba83fe9a9d479f70c624739256983dc8f01cccfa`, and
its semantic self-hash is
`f84c29313cf36449e9f4ff2c68ecd00473f5841037db086022b7abc38db53872`.
The B1 spot-cutoff remediation changed the executable closure, so this append-only freeze
supersedes v5 without altering the Phase 8 cells. The audit verifies the runtime closure
and registry binding before replay or measurement.
...
`artifacts/phase8_bridge/dispersion_audit_20260831_v11.json`. Its writer refuses any
registered output path. The artifact records both D/V references, the verified current
D/V executable closure, the independently frozen audit/replay producer, fixed source
identities, the exact replay, the unresolved historical-producer identity, the unavailable
historical panel bytes and `sealed_store_reopened = false`.
