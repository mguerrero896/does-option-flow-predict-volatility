...
**Current deliverables — read these:**
...
| [`phase8a_exploratory_bridge_addendum_v13.md`](phase8a_exploratory_bridge_addendum_v13.md) | Current post-cutoff Phase 8A interpretation and same-session remediation sensitivity |
...
The numbered `phase8a_exploratory_bridge_addendum_v1` … `v12` files are superseded drafts
of the current `v13`. They are kept because each was registered at a specific SHA-256 and
...
**Only `v13` is current.** The three files in the table above are hash-pinned under
`current_report` in [`data/CANONICAL_STATE.json`](../data/CANONICAL_STATE.json), which
governs whenever a document disagrees with it.

Presentation and defense material must be regenerated from the current canonical state
rather than reused from the historical packages, whose numerical claims were withdrawn.
Those claims and the reasons for withdrawal are recorded in
