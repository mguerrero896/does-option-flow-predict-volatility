...
**Study identifier**: `001-pit-options-rv30`
...
- Q: What is the frozen target-blind B1v3 specification? → A: The requirements in Phase 6
  of this document are the public design contract. Implementation MUST preserve its feature
  geometry, exact-lag rules, coverage gates, pristine
...
The requirements-consistency report has zero critical contradictions and its preregistration has been
...
- **FR-082**: Raw ZIPs, Parquet data and provider caches MUST reside under the
  configured `<DATA_ROOT>` paths. The run MUST stop before another acquisition batch if projected
