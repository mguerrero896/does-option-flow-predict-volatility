-- Applied to Supabase as migration 20260826180409 loader_run_scoped_staging.
-- Remote statements MD5: 066ad7db1e62b20cec39347e8ef24cbb.
-- Do not edit; corrections are new migrations.
-- ---- verbatim applied SQL follows ----
-- PENDING OWNER-APPLIED MIGRATION. Do not place in migrations/ until Supabase records it.
-- Each loader run owns its staging rows, so concurrent runs cannot truncate or promote
-- each other's data. The service-role privileges are explicit rather than ambient.

alter table public.dev_training_all_origins__staging add column if not exists load_id uuid;
alter table public.dev_training_common__staging add column if not exists load_id uuid;
alter table public.c1_development_forecasts__staging add column if not exists load_id uuid;
alter table public.c5_frozen_evaluation_forecasts__staging add column if not exists load_id uuid;
alter table public.b1v3_features__staging add column if not exists load_id uuid;
alter table public.b2_mechanism_forecasts__staging add column if not exists load_id uuid;

alter table public.dataset_loads enable row level security;
alter table public.dev_training_all_origins__staging enable row level security;
alter table public.dev_training_common__staging enable row level security;
alter table public.c1_development_forecasts__staging enable row level security;
alter table public.c5_frozen_evaluation_forecasts__staging enable row level security;
alter table public.b1v3_features__staging enable row level security;
alter table public.b2_mechanism_forecasts__staging enable row level security;

revoke all on public.dataset_loads from public, anon, authenticated;
revoke all on public.dev_training_all_origins__staging, public.dev_training_common__staging,
  public.c1_development_forecasts__staging, public.c5_frozen_evaluation_forecasts__staging,
  public.b1v3_features__staging, public.b2_mechanism_forecasts__staging from public, anon, authenticated;
grant select on public.dataset_loads to service_role;
grant select, insert, delete on public.dev_training_all_origins__staging,
  public.dev_training_common__staging, public.c1_development_forecasts__staging,
  public.c5_frozen_evaluation_forecasts__staging, public.b1v3_features__staging,
  public.b2_mechanism_forecasts__staging to service_role;
...
begin
  ...
  delete from public.contrast_results;
