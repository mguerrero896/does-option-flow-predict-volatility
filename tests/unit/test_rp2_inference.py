...
def test_mde_sizing_reuses_the_inference_long_run_variance_contract() -> None:
    values = np.random.default_rng(650).normal(size=80)
    long_run = newey_west_variance(values, lags=SESSION_BLOCK_LENGTH)

    assert minimum_detectable_effect(values) == pytest.approx(
        minimum_detectable_effect_from_long_run_variance(long_run, values.size)
    )
