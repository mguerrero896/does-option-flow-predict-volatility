...
def test_artifact_integrity_normalises_only_tracked_text_line_endings(tmp_path: Path) -> None:
    text = tmp_path / "result.json"
    text.write_bytes(b'{"ok": true}\r\n')
    expected = tmp_path / "expected.json"
    expected.write_bytes(b'{"ok": true}\n')
    binary = tmp_path / "panel.parquet"
    binary.write_bytes(b"PAR1\r\n")
    binary_lf = tmp_path / "panel-lf.parquet"
    binary_lf.write_bytes(b"PAR1\n")

    assert artifact_digest(text) == artifact_digest(expected)
    assert artifact_digest(binary) != artifact_digest(binary_lf)
