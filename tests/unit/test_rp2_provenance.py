...
def test_stored_path_can_be_public_and_repo_relative(tmp_path: Path) -> None:
    source = tmp_path / "artifacts" / "input.json"
    source.parent.mkdir()
    source.write_text("{}", encoding="utf-8")
    record = describe_input(
        source, provider="derived", stored_path="artifacts/input.json"
    )
    assert record.path == "artifacts/input.json"
    assert verify_inputs({"input": record}, base_path=tmp_path) == []
