...
def test_phase4a_profile_has_no_duplicate_join_suffix_columns() -> None:
    ...
    if not profile_path.exists():
        # Deliberate deferral, not a build step: this artifact lives only in the
        # local RP2-v2 lineage and has never existed on the public mirror, so on
        # this lineage the test is a documented no-op, the same regime as the
        # INTERNAL_DOC_NOT_ON_PUBLIC_MIRROR contract tests.
        pytest.skip("PHASE4A_ARTIFACT_NOT_ON_THIS_LINEAGE: common_matrix_profile_v1.json")
...
def test_phase4a_secret_scan_records_history_gate() -> None:
    ...
    if not scan_path.exists():
        pytest.skip("PHASE4A_ARTIFACT_NOT_ON_THIS_LINEAGE: secret_scan_v1.json")
