...
def _fit(name: str, design: np.ndarray, target: np.ndarray, train: np.ndarray) -> np.ndarray:
    """Exercise every family through the production route with real grouped sessions."""

    sessions = np.arange(target.size, dtype=np.int64) // 10
    return fit_ladder_model(name, design, target, train, sessions=sessions)
...
def test_every_model_predicts_a_positive_variance_for_all_rows(name: str) -> None:
    ...
    forecast = _fit(name, design, target, train)
...
def test_the_qlike_booster_beats_the_log_mse_booster_on_qlike() -> None:
    ...
    aligned = float(
        qlike_losses(
            target[held_out], _fit("lightgbm_qlike", design, target, train)[held_out]
        ).mean()
    )
    log_mse = float(
        qlike_losses(target[held_out], _fit("lightgbm", design, target, train)[held_out]).mean()
    )
...
def test_no_model_tunes_on_validation_sessions(name: str) -> None:
    ...
    design, target, train = _panel()
    baseline = _fit(name, design, target, train)
    corrupted = target.copy()
    corrupted[~train] *= 1e6
    after = _fit(name, design, corrupted, train)
