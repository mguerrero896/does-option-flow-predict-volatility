...
def test_a_log_transform_refuses_to_place_a_non_positive_value_on_the_log_scale() -> None:
    """No input may be given a finite log-scale coordinate the log scale did not define.

    Clamping to ``VARIANCE_FLOOR`` gave every zero and every negative the same finite
    coordinate, ``log(1e-12) = -27.63``, which for a count of trades or a sum of dollars is
    tens of standard deviations below the body of the column and is indistinguishable from
    a measured extreme. The floor is a legitimate guard on a *variance*, which is bounded
    below by zero for numerical reasons; it is not a value a count may take. A
    non-positive input is absent on this scale, and `fit_preprocessor` is what decides what
    to do about an absence.
    """

    values = np.array([-100.0, -1e-30, 0.0, VARIANCE_FLOOR, 1e-6, 100.0])
    logged = transform_column(values, "log")
    assert not np.isfinite(logged[:3]).any()
    assert np.isfinite(logged[3:]).all()
    assert logged[3] == pytest.approx(np.log(VARIANCE_FLOOR))
    assert logged[5] == pytest.approx(np.log(100.0))
...
def test_transforms_behave_as_declared() -> None:
    ...
    logged = transform_column(values, "log")
    assert not np.isfinite(logged[0])
    assert not np.isfinite(logged[1])
