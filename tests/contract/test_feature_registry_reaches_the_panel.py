...
Both checks fail when the panel is absent, unless MDS650_PANEL_GUARD_MAY_SKIP=1 says the
runner genuinely has no licensed derived data. That claim now lives in `ci.yml` where it can
be read, instead of being inferred from a missing file.
"""
...
DECLARED_DIAGNOSTICS: frozenset[str] = frozenset(
    {
        ...
        # Snapshot/PIT counters consumed by the scorecard, never by a fitted design.
...
# Histogram buckets are diagnostics too; their cardinality belongs to the producer's
# versioned bin edges, so enumerating every suffix here would make this declaration stale.
DECLARED_DIAGNOSTIC_PREFIXES: tuple[str, ...] = ("b1_quote_age_bin_", "b2_latency_bin_")
...
def test_no_panel_column_is_a_feature_nobody_registered() -> None:
    ...
    for name, path, _ in PANELS:
        ...
        candidates = {
            ...
            and not column.startswith(DECLARED_DIAGNOSTIC_PREFIXES)
        }
