...
def inference(document: str) -> dict:
    ...
    if not path.is_file():
        # Not a skip. A run's inference artifact is a small JSON and is committed with the
        # run -- origin/main carries seventy-seven files under artifacts/rp2_v3 -- so its
        # absence does not mean "this checkout is thin", it means the document names a run
        # nobody committed. That is the drift this contract exists to catch, and skipping
        # here reported it as agreement.
        pytest.fail(
            f"VERDICT_RUN_UNVERIFIED: the document is measured on `{run_id}` and "
            f"{path.relative_to(REPO).as_posix()} does not exist, so every figure on the "
            f"page is UNVERIFIED, not confirmed. Commit that run's artifacts, or point the "
            f"document at a run that is committed."
        )
...
def test_alpha_contract_is_not_mixed_with_future_spending(document, inference) -> None:
    assert inference["inference_alpha"] == pytest.approx(0.05)
    assert inference["inference_power"] == pytest.approx(0.80)
    assert inference["future_alpha_budget"] == pytest.approx(0.05 / (3 * 4))
    assert inference["alpha_budget_scope"] == (
        "FUTURE_CAMPAIGNS_ONLY_POST_HOC_SENSITIVITY_HERE"
    )
    assert "MDEs and session requirements share the exploratory" in document
    assert "It is not the decision threshold for D/V" in document
