...
#: The frozen specification declared by the public research contract.
...
#: The twelve registered gate branches, in their binding order.
...
def _fields_by_group(document: str) -> dict[str, set[str]]:
    ...
    Grouping is part of the contract, not presentation: the schema assigns each metric
...
def test_implementation_status_tracks_the_twelve_gates_in_order() -> None:
    ...
    assert positions == sorted(positions), (
        "the twelve gates must appear in their registered binding order"
    )
