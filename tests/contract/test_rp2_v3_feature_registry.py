...
def test_the_core_coverage_floors_hold_on_the_real_panels() -> None:
    ...
    # `if not built` also passed vacuously on a PARTIAL build: two of the three core panels
    # present narrowed the check to two and emitted no skip at all. Each core panel is now
    # required by name.
    built = []
