...
def test_target_horizon_document_keeps_rv30_frozen_after_role_repair() -> None:
    artifact = json.loads(ARTIFACT.read_text(encoding="utf-8"))
    ...
    assert validation["60"]["oos_log_r2"] > validation["30"]["oos_log_r2"]
    document = DOCUMENT.read_text(encoding="utf-8")
    assert "not authority to replace the frozen RV30" in document
