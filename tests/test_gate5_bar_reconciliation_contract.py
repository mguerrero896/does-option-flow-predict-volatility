...
def test_bar_label_convention_pinned() -> None:
    if not ARTIFACT.exists():
        # The artifact is git-tracked next to its .sha256, so it is present in every
        # checkout. Absence means it was deleted or renamed, which is the thing this
        # contract pins -- and skipping reported that as the convention holding.
        pytest.fail(
            f"GATE5_ARTIFACT_UNVERIFIED: {ARTIFACT.name} is tracked and missing, so the "
            f"bar-label convention is UNVERIFIED, not pinned."
        )
