...
def test_session_minute_expression_survives_the_int8_trap() -> None:
    """9:30 NY must be minute 0 and 15:59 minute 389 — the uncasted spelling gave
    -512 and -635 (Int8 overflow, measured on polars 1.42.1)."""
    ...
    frame = pl.DataFrame(
        {
            "forecast_origin_utc": [
                datetime(2026, 8, 21, 13, 30, tzinfo=UTC),  # 09:30 NY (EDT)
                datetime(2026, 8, 21, 16, 0, tzinfo=UTC),  # 12:00 NY
                datetime(2026, 8, 21, 19, 59, tzinfo=UTC),  # 15:59 NY
                datetime(2026, 1, 21, 14, 30, tzinfo=UTC),  # 09:30 NY (EST)
            ]
        }
    )
    minutes = frame.select(har.session_minute_expression().alias("m"))["m"].to_list()
    assert minutes == [0, 150, 389, 0]
