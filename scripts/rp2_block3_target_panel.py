...
def rows_by_role(panel: pl.DataFrame) -> dict[str, int]:
    """Count roles once so labels cannot swap between independent results."""

    return {str(role): int(count) for role, count in panel["role"].value_counts().iter_rows()}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    document: dict[str, object] = {
        ...
        "rows_by_role": rows_by_role(panel),
