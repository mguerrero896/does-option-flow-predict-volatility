...
def evaluate() -> None:
    ...
    custody_path = ROOT / "artifacts/target_blind_v22/successor_custody_audit_v2.json"
    custody = json.loads(custody_path.read_text())
    if custody["result_file_sha256"] != sha256_file(result_path):
        raise ValueError("B2_REPAIR_CUSTODY_RESULT_MISMATCH")
    names = {
    ...
    # The result records a semantic preregistration hash; custody records file bytes.
    expected = {name: original["hashes"][key] for name, key in names.items()}
    expected["runtime_preregistration.json"] = custody["verified_content_addressed_payloads"][
    ...
    protocol: dict[str, Any] = {
        ...
        "source_hashes": hashes,
        "custody_audit_sha256": sha256_file(custody_path),
