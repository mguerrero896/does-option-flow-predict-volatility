...
def _configure_paths() -> None:
    ...
    global PRODUCTION_ROOT, DATA_ROOT, STORE, LOGS, ALERT
...
def _alert(message: str) -> None:
    assert LOGS is not None and ALERT is not None
...
def _checkpoint(session_dir: Path, stage: str) -> None:
    payload = {"stage": stage, "utc": dt.datetime.now(dt.UTC).isoformat()}
    tmp = session_dir / "collector_checkpoint.json.tmp"
    tmp.write_text(json.dumps(payload), encoding="utf-8")
    tmp.replace(session_dir / "collector_checkpoint.json")
...
def main(argv: list[str] | None = None) -> None:
    ...
    _configure_paths()
    assert DATA_ROOT is not None and STORE is not None
    ...
    if free_gb < MIN_FREE_GB:
        ...
        raise SystemExit(1)
    session = (
        dt.date.fromisoformat(arguments.session) if arguments.session else last_closed_session()
    )
    if session is None:
    ...
    if not arguments.dry_run and counter_path.exists():
        counter = json.loads(counter_path.read_text(encoding="utf-8"))
        if int(counter.get("reads", 0)) != 0:
            _alert("PHASE9_SEAL_BROKEN: counter reads is non-zero")
            raise SystemExit(1)
        if len(counter.get("sessions", [])) >= TARGET_SESSIONS:
            print(f"[phase9] collection already complete ({TARGET_SESSIONS}/{TARGET_SESSIONS})")
            return
    ...
    try:
        ...
        _alert(f"session {session}: COLLECTOR_FAILED stage={stage} error={type(error).__name__}")
        raise
    ...
    manifest = {
        ...
        "bars_by_asset": {asset: bars.filter(pl.col("asset") == asset).height for asset in ASSETS},
        "tape_bytes": tape.stat().st_size,
        "quote_rows": quotes.height,
        "quote_ok": ok_quotes,
        "quote_ok_by_asset": {
            asset: quotes.filter((pl.col("asset") == asset) & (pl.col("status") == "OK")).height
            for asset in ASSETS
        },
        "sha256": {
