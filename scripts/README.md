...
| `scripts/audit_successor_holdout_exposure_v1.py` | CONTROLLED | Audit successor-holdout exposure from frozen target-free metadata without opening an external panel or outcome payload. |
...
| `scripts/verify_omitted_contracts.py` | CONTROLLED | Re-run the exact omitted contracts from a reviewed public JUnit report with explicit evidence roots; fail on missing, failed or skipped checks. |
...
| `scripts/harvest_uw_latency_campaign.py` | Regenerate immutable target-blind UW campaign/state snapshots, including custody-safe receipt-hour aggregates. |
...
| `scripts/audit_b2_history_repair_v1.py` | Frozen producer of the recorded B2 history-repair feature impact and paired evaluation; rerunning its data-bearing commands requires the original protocol authorization. |
...
| `scripts/build_pit_v22_successor_v2_closeout.py` | Reproduce the private successor-v2 custody hashes and emit the sanitized immutable audit/claim closeout. |
...
| `scripts/run_pit_v22_successor_once.py` | Historical one-shot runner: v1 failed before OOS and v2 completed one authorized read; neither run may be repeated. |
