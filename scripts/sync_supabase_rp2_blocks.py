...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Guard BEFORE reading anything: a review noted the artefacts were opened
    # first, so a sealed-named source would already have been read by the time
    # the gate ran. The gate now covers the sources as well as the targets.
    guard_sealed_access(
        ["ingestion_runs", "ingestion_inputs", "rp2_block_results",
         "artifacts/rp2_v3", "data/FROZEN_ARTIFACTS.json"],
        operation="rp2 run-register sync (pre-read)",
    )
    payload = rows()
    run, run_inputs = run_manifest(str(args.run_id))
    guard_sealed_access(
        ["ingestion_runs", "ingestion_inputs", "rp2_block_results",
         *[row["document"] for row in payload]],
        operation="rp2 run-register sync",
    )
    for row in payload:
