...
#: The one store whose alerts are real. A desktop popup is global state that no
#: path rebinding can contain, so it is gated on this rather than fired
#: unconditionally: any run against a redirected store - a test, a dry run, a
#: sandbox - is silent on the operator's screen by construction.
...
def _alert(message: str) -> None:
    ...
    # The file records every shortfall wherever the store points; only the desktop
    # notification is withheld, and only when this is not the production store. The
    # contract suite fired eleven of these at the operator in two minutes, each one a
    # fixture describing a session already known and already over.
...
def _latest_session(now_ny: dt.datetime | None = None) -> dt.date | None:
    calendar = exchange_calendars.get_calendar("XNYS")
    now_ny = now_ny or dt.datetime.now(NY)
    probe = now_ny.date()
    if calendar.is_session(probe.isoformat()) and now_ny.time() < CLOSE_NY:
