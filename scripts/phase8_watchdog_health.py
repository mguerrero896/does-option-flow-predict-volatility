...
def main() -> None:
    ...
    log = arguments.log or effective_data_root() / "logs" / "phase8_watch.log"
    # Machine-LOCAL yesterday, deliberately — the one sanctioned exception to the
    # exchange-clock rule (mds650.exchange_clock). The watchdog is a Task Scheduler
    # job firing at 20:00 machine-local and stamping its log with machine-local
    # dates, so the only clock that names its runs correctly is the machine's own.
    # An exchange clock here would audit the WRONG line for ~14 h of every day.
    expected = (
    ...
    if not arguments.date:
        print(
            f"[phase8-watchdog] auditing {expected.isoformat()} "
            "(machine-local yesterday; the watchdog schedules and logs in local time)"
        )
    found = reasons(log, expected)
    for reason in found:
