...
def _window_sum(values: FloatArray, origins: npt.NDArray[np.int64], window: int) -> FloatArray:
    """Sum ``values`` over ``[origin - window + 1, origin]``, NaN only where one is absent.

    `build_session_grid` writes NaN into the volume of a minute whose price moved on a
    recorded volume of zero, because a price cannot move without trades. That absence is
    real and must propagate - but only into the windows that contain it. Summing the raw
    series through `cumsum` propagates it much further: every prefix from that minute on
    is NaN, so every *later* origin loses its window too, whatever its own 30 minutes
    hold. Rebuilt over the six bar stores, 7,094 of 152,954 development origins lost
    `volume_30` and `dollar_volume_30` where only 1,522 have an absent minute inside
    their own window, and validation lost none either way. The 5,572 needlessly imputed
    ones carry the amplification's signature: on all 169 affected session-assets the lost
    origins are a contiguous suffix reaching the close. Counting the absences in the
    window separately keeps the honest NaN and drops the inherited one.
    """

    absent = np.isnan(values)
    prefix = np.concatenate([[0.0], np.cumsum(np.where(absent, 0.0, values))])
    gaps = np.concatenate([[0.0], np.cumsum(absent.astype(np.float64))])
    total = prefix[origins + 1] - prefix[origins + 1 - window]
    inside = gaps[origins + 1] - gaps[origins + 1 - window]
    return np.asarray(np.where(inside > 0.0, np.nan, total), dtype=np.float64)
...
def evaluate_role(
    ...
    *,
    ...
    core_finite = np.asarray(np.isfinite(core_design).all(axis=1), dtype=np.bool_)
    ...
    market_finite = np.asarray(np.isfinite(market_design).all(axis=1), dtype=np.bool_)
