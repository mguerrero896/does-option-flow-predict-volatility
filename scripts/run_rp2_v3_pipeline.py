...
from mds650.config import rp2_store_root  # noqa: E402
...
def gated_data_root() -> Path:
    """Resolve licensed artifacts without requiring them in a public worktree."""

    return Path(os.environ.get("MDS650_EVIDENCE_ROOT", ROOT))


def _peak_memory_bytes(process: subprocess.Popen[bytes]) -> int:
    """Wait for a step and sample the full process tree's resident memory.

    This is a sampled RSS sum: shared pages can count once per process and allocations
    shorter than 50 ms can be missed. It still measures the executing tree instead of the
    finished launcher, which previously reported about 4 MiB for steps observed near 3 GiB.
    """
...
def model_config_digest() -> str:
    ...
    configuration = {
        ...
        "lightgbm_max_rounds": ladder._LIGHTGBM_MAX_ROUNDS,
        "lightgbm_early_stopping_rounds": ladder._LIGHTGBM_EARLY_STOPPING_ROUNDS,
        "lightgbm_inner_validation_share": ladder._LIGHTGBM_INNER_VALIDATION_SHARE,
...
def assert_producers_share_the_mask(run_dir: Path, roles: Sequence[str]) -> dict[str, str]:
    """The ladder and incremental inference used the same rows and fitted models.
    ...
    not the comparison either artifact claims to be.  For every model/set pair, compare
    the exact forecast and loss vectors too.  Boosted fits additionally have to agree on
    the full inner-validation protocol; an equal outer mask cannot prove that rounds were
    selected from the same sessions.
    """
    ...
    agreed: dict[str, str] = {}
    for role in roles:
        ladder_role = ladder.get(role, {})
        inference_role = inference.get(role, {})
        if not isinstance(ladder_role, dict) or not isinstance(inference_role, dict):
            raise SystemExit(f"RP2_RUN_PRODUCER_ROLE_MISSING:{role}")
        left = ladder_role.get("evaluation_mask_sha256")
        right = inference_role.get("evaluation_mask_sha256")
        if left is None or right is None or left != right:
            raise SystemExit(
                f"RP2_RUN_MASK_DISAGREEMENT:{role}:{str(left)[:12]}!={str(right)[:12]}"
            )
        ladder_models = ladder_role.get("models")
        inference_models = inference_role.get("model_provenance")
        if not isinstance(ladder_models, dict) or not isinstance(inference_models, dict):
            raise SystemExit(f"RP2_RUN_MODEL_PROVENANCE_MISSING:{role}")
        primary_models = set(PRIMARY_MODELS)
        if not primary_models.issubset(ladder_models) or set(inference_models) != primary_models:
            raise SystemExit(f"RP2_RUN_MODEL_PROVENANCE_DISAGREEMENT:{role}")
        for model_name in sorted(primary_models):
            ladder_model = ladder_models[model_name]
            inference_model = inference_models[model_name]
            if not isinstance(ladder_model, dict) or not isinstance(inference_model, dict):
                raise SystemExit(f"RP2_RUN_MODEL_PROVENANCE_MISSING:{role}:{model_name}")
            for field, code in (
                ("forecast_sha256", "FORECAST_HASH"),
                ("loss_sha256", "LOSS_HASH"),
            ):
                ladder_hashes = ladder_model.get(field)
                inference_hashes = inference_model.get(field)
                if not isinstance(ladder_hashes, dict) or not isinstance(inference_hashes, dict):
                    raise SystemExit(f"RP2_RUN_{code}_MISSING:{role}:{model_name}")
                if set(ladder_hashes) != set(inference_hashes):
                    raise SystemExit(f"RP2_RUN_{code}_SET_DISAGREEMENT:{role}:{model_name}")
                for set_name in sorted(ladder_hashes):
                    left_hash = ladder_hashes[set_name]
                    right_hash = inference_hashes[set_name]
                    if not isinstance(left_hash, str) or not isinstance(right_hash, str):
                        raise SystemExit(f"RP2_RUN_{code}_MISSING:{role}:{model_name}:{set_name}")
                    if left_hash != right_hash:
                        raise SystemExit(
                            f"RP2_RUN_{code}_DISAGREEMENT:{role}:{model_name}:{set_name}"
                        )
            if model_name in BOOSTED_LADDER:
                ladder_protocols = ladder_model.get("boosting_rounds")
                inference_protocols = inference_model.get("boosting_rounds")
                if not isinstance(ladder_protocols, dict) or not isinstance(
                    inference_protocols, dict
                ):
                    raise SystemExit(f"RP2_RUN_BOOSTED_PROTOCOL_MISSING:{role}:{model_name}")
                if set(ladder_protocols) != set(inference_protocols):
                    raise SystemExit(
                        f"RP2_RUN_BOOSTED_PROTOCOL_SET_DISAGREEMENT:{role}:{model_name}"
                    )
                for set_name in sorted(ladder_protocols):
                    left_protocol = ladder_protocols[set_name]
                    right_protocol = inference_protocols[set_name]
                    if not isinstance(left_protocol, dict) or not isinstance(right_protocol, dict):
                        raise SystemExit(
                            f"RP2_RUN_BOOSTED_PROTOCOL_MISSING:{role}:{model_name}:{set_name}"
                        )
                    if left_protocol != right_protocol:
                        raise SystemExit(
                            f"RP2_RUN_BOOSTED_PROTOCOL_DISAGREEMENT:{role}:{model_name}:{set_name}"
                        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        args.data_root = args.data_root or rp2_store_root()
    ...
    steps.append(
        StepRecord(
            ...
            peak_memory_bytes=_own_peak_memory_bytes(),
            artifacts={"input_manifest.json": artifact_digest(run_dir / "input_manifest.json")},
            content={
        ...
        if args.skip_panels:
            ...
            steps.append(
                StepRecord(
                    ...
                    peak_memory_bytes=0,
                    artifacts={
                        output: artifact_digest(run_dir / output) for output in step.outputs
                    },
                    content={
    ...
    for name, produce in internal_steps:
        ...
        steps.append(
            StepRecord(
                ...
                peak_memory_bytes=_own_peak_memory_bytes(),
                artifacts={artifact.name: artifact_digest(artifact)},
    ...
    steps[:] = [
        StepRecord(
            ...
            artifacts={masks_path.name: artifact_digest(masks_path)},
    ...
    steps.append(
        StepRecord(
            ...
            peak_memory_bytes=_own_peak_memory_bytes(),
            artifacts={
                "scorecard.json": artifact_digest(run_dir / "scorecard.json"),
                "scorecard.md": artifact_digest(run_dir / "scorecard.md"),
            },
            content={
