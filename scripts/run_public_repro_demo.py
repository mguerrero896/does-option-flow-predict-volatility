"""Public methodological smoke demo on redistributable synthetic data.

Computational reproducibility is not data reproducibility (decision 63): a third
party without provider licenses cannot rebuild the licensed panels. This bounded
demo exercises representative analytical primitives from a clean clone — no provider
keys, no external drives, no licensed bytes:
...
It does not perform provider acquisition, raw-payload normalization, production B0/B1/B2
builders, the RP2 runner, manifest generation, Supabase synchronization, or publication.
Passing it is not evidence that the production pipeline ran.
"""
