...
def describe_treatment_batteries() -> dict[str, object]:
    """State which joint test is primary and how the two batteries actually relate.

    The block fits two overlapping batteries and they disagree in validation. Until this
    was recorded, which one is the primary test lived in a source comment alone, so a
    reader of `dml.json` could quote the secondary `full_V` rejection as "B2 contributes
    in validation" with nothing in the artifact to contradict them. `full` is neither a
    superset of `core` nor the whole of B2: it resolves to `B2_CORE`, twelve channels of
    the fifty-six `B2_RICH` declares. Nothing here chooses the primary; `core` is the
    battery the block has always designated, and this only writes it down.
    """

    primary = tuple(CORE_TREATMENTS)
    secondary = tuple(B2_FEATURES)
    only_primary = sorted(set(primary) - set(secondary))
    only_secondary = sorted(set(secondary) - set(primary))
    nested = not only_primary
    return {
        "primary": "core",
        "primary_keys": ["core_D", "core_V"],
        "primary_count": len(primary),
        "primary_treatments": list(primary),
        "secondary": "full",
        "secondary_keys": ["full_D", "full_V"],
        "secondary_count": len(secondary),
        "secondary_treatments": sorted(secondary),
        "secondary_registry_set": "B2_CORE",
        "secondary_is_complete_b2": False,
        "b2_rich_count": len(feature_map("B2_RICH")),
        "nested": nested,
        "relation": "NESTED" if nested else "OVERLAPPING_NOT_NESTED",
        "shared_count": len(set(primary) & set(secondary)),
        "only_in_primary": only_primary,
        "only_in_secondary": only_secondary,
        "reading_rule": (
            "The primary joint test is core_D / core_V. Because the secondary battery is "
            "not a superset of the primary one, full_D / full_V are a different test on a "
            "different battery, not a robustness check of the primary result, and a "
            "rejection there does not license a claim about the primary contrast."
        ),
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    document: dict[str, object] = {
        ...
        # Which of the two joint tests is the primary one, and why the other is not a
        # robustness check of it. A comment in this file cannot be read from the artifact.
        "treatment_batteries": describe_treatment_batteries(),
    }
    for role in ("D", "V"):
