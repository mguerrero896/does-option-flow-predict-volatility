...
SPEC_VERSION = "rp2-v3"
CANONICAL_STATE = ROOT / "data" / "CANONICAL_STATE.json"
...
def assert_publication_eligible(
    manifest: dict[str, Any], *, state_path: Path = CANONICAL_STATE
) -> None:
    """Require an explicit, current repository authorization before the live RPC."""

    state = _read(state_path)
    bundle = state.get("scientific_bundle", {})
    eligibility = bundle.get("eligibility", {})
    canonical = state.get("canonical_results", {})
    run_id = str(manifest.get("run_id", ""))
    reasons = eligibility.get("reasons", [])
    if (
        bundle.get("run_id") != run_id
        or canonical.get("status") != "CURRENT_ELIGIBLE_RESULT"
        or eligibility.get("status") != "ELIGIBLE_FOR_PUBLICATION"
        or eligibility.get("safe_to_reconcile_existing_results") is not True
        or reasons
    ):
        raise SystemExit(
            "RP2_PUBLISH_NOT_ELIGIBLE:"
            f"{run_id}:{eligibility.get('status', 'MISSING')}:"
            f"{','.join(map(str, reasons)) or 'NO_RECORDED_REASON'}"
        )
...
def assert_study_window_decided(
    ...
    The repository records two historical candidates: twelve months from 2025-07-21, and
    the partition used by every artifact, 2024-08-02 through 2026-07-17. A code change cannot
    ...
    blocks = [
        {
            ...
            "document": "docs/rp2_v3/REBUILD_GUIDE.md",
...
def main(argv: Sequence[str] | None = None) -> int:
    guard_sealed_access(
        ["rp2_block_results", "rp2_contrast_results", "rp2_power_results",
         "rp2_extension_results", "ingestion_runs", "artifacts/rp2_v3"],
        operation="rp2-v3 publication",
    )
    ...
    # never used.
    manifest = _read(args.run_root / "run_manifest.json")
    assert_published_at_the_run_commit(manifest)
    ...
    assert_publication_eligible(manifest)
