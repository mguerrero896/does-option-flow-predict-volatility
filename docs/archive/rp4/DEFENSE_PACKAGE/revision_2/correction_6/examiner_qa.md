...
**Fuente original y alcance de la comparación.** Se contrastaron completas las copias locales inglesa y española de la propuesta fechada **2026-07-26**. Las secciones y el texto coinciden con los compromisos examinados; su fecha es la declarada en el documento, no una marca temporal independiente ni una autenticación de la versión entregada a la universidad. Los [extractos literales y hashes de los originales](../correction_4/proposal_source_extract.json) permiten comprobar las citas sin publicar la portada ni identificadores personales. La ecuación original define RV30 como la suma de treinta retornos logarítmicos de un minuto al cuadrado, sin raíz ni anualización; el [productor del objetivo](../../../../../../src/mds650/rp2/realized.py) conserva esa definición. La [evidencia de alineación](../correction_4/proposal_alignment_evidence.json) enlaza cada constatación con archivos y selectores; «pendiente» no significa necesariamente archivo inexistente.

### Primero, las preguntas originales a RV30

Las preguntas se citan literalmente de la sección **2.3** de la [propuesta española](../correction_4/proposal_source_extract.json); las respuestas usan la primaria RV30 de v3, con H1→H2 unilateral dentro de cada familia. Son resultados del desarrollo retrospectivo, no confirmación independiente. El [resumen guardado](../../../../../../artifacts/rp4_v3_b2/summary.json) identifica la [especificación efectiva de ventanas vacías](../../../../../../artifacts/rp4_v3_a1_empty_window/specification.json).

| Pregunta original | Respuesta a RV30 antes de la extensión | Artefacto que permite examinarla |
| --- | --- | --- |
| RQ1: «¿La información convencional del estado de opciones mejora RV30 frente a un benchmark del mercado subyacente?» | **Sí**, B1 > B0 en ambas familias: p unilateral **0,0439** lineal y **0,0053** árboles. El primer rechazo es marginal y depende de la regla inferencial declarada. | [Primaria v3](../../../../../../artifacts/rp4_v3_b2/summary.json) · [Sensibilidad y búsqueda histórica](#26-multiplicidad-entre-versiones-qué-sobrevive-bajo-la-regla-bilateralholm-antigua) |
| RQ2: «¿La actividad derivada de operaciones agrega valor predictivo incremental sobre ese benchmark convencional de opciones?» | **No se detecta al 5 %**: H2 tiene p **0,0525** lineal y **0,6631** árboles. El estimador lineal es favorable pero el umbral no se cruza; no rechazo no demuestra efecto cero, equivalencia ni absorción causal del flujo. | [Contrastes y decisiones v3](../../../../../../artifacts/rp4_v3_b2/summary.json) |
| RQ3: «¿Las mejoras son estables entre activos, segmentos horarios, regímenes de volatilidad y supuestos temporales conservadores?» | **Parcial**: B1 tiene signo favorable en los seis activos de ambas familias, pero ninguno de los **12** contrastes secundarios por activo rechaza tras Holm dentro de su subconjunto de cuatro contrastes; el menor p ajustado es **0,0624**. Hay desgloses horarios/calendario y circunstancias documentadas de aprendizaje, instrumento y vencimientos, sin acreditar toda la estabilidad propuesta ni variantes PIT. | [Secundarios RV30](../../../../../../artifacts/rp4_v3_b2/summary.json) · [Perfiles guardados](../../../../../../artifacts/rp4_closeout_audit/descriptive_profiles.csv) · [Auditoría de vencimientos](../../../../../../artifacts/rp4_market_audit/REPORT.md) |

El periodo de aprendizaje se documenta con entrenamiento expansivo y perfiles por tamaño de entrenamiento; las interrupciones atribuidas al proveedor se distinguen de las ventanas vacías efectivamente censadas y del hueco aceptado sin relleno; la ruptura de vencimientos se sitúa en la primera presencia registrada el **2026-01-26**. Son circunstancias descriptivas, no tres regímenes estadísticos predefinidos de volatilidad ni explicaciones causales del resultado. El inventario secundario inspeccionado contiene segmentos horarios, calendario y proxies de flujo/gamma, pero no acredita un contraste específico de regímenes de volatilidad ni de cortes PIT alternativos. La etiqueta guardada `last_hour` usa `origin_minute >= 300`, de modo que tampoco se interpreta como los últimos sesenta minutos exactos de la sesión regular. [Perfiles y límites](../../../../../../artifacts/rp4_closeout_audit/REPORT.md) · [Definición de estratos](../../../../../../artifacts/rp4_v3_code/aggregate_v3.py) · [Primera presencia por activo](../../../../../../artifacts/rp4_market_audit/summary.json).

### Después, la extensión de horizonte y las desviaciones

RV15 primario y RV5 secundario se presentan como **extensión pre-declarada v4**, después de la respuesta anterior a RV30. El antecedente condicional declara **2026-09-07 20:20 Australia/Sydney**; el registro ejecutable v4 se congeló el **2026-09-08T02:37:21.501511+10:00**, antes de sus evaluaciones. La motivación registrada es la hipótesis de vida corta del flujo y de cobertura en minutos, no un mecanismo demostrado. El cambio de horizonte primario después de leer RV30 es una **DESVIACIÓN DECLARADA**, con secuencia documental local y búsqueda histórica no corregida. La cronología completa conservada en la pregunta sobre las versiones distingue antecedente, congelación, verificación y ejecución; el sello no es de un tercero. [Antecedente literal](../../../../../rp4/predeclaration_v4_text.md) · [Freeze v4](../../../../../../artifacts/rp4_v4_a1/freeze_manifest.json) · [Cronología](../correction_2/examiner_evidence.json).

| Compromiso de la propuesta | Entrega observada | Estado y límite | Evidencia |
| --- | --- | --- | --- |
| Pregunta incremental y B0 ⊂ B1 ⊂ B2; barras de un minuto, estado de contratos/cotizaciones y actividad por operación | Se conservan las capas anidadas precio/historia, estado/superficie y composición/actividad/desbalance. | **ALINEADO** en pregunta y tipos de información; no implica tres proveedores independientes: superficie y flujo pueden compartir operaciones y campos del proveedor. | [Propuesta](../correction_4/proposal_source_extract.json) · [Entradas y variables](../../../../../rp4/specification_v1.md) · [Registro efectivo](../../../../../../artifacts/rp4_v3_a1_empty_window/specification.json) |
| Orígenes cada cinco minutos; walk-forward cronológico expansivo; QLIKE primario e intervalos por bloques de días | Rejilla de cinco minutos y entrenamiento pasado; bootstrap circular de **5 sesiones / 9.999 réplicas**. | **ALINEADO**; los orígenes no se tratan como observaciones independientes y sólo se usan claves elegibles. La propuesta no fijaba ese tamaño ni ese número de réplicas: son decisiones del registro de ejecución. | [Rejilla](../../../../../../scripts/rp2_block4_b0_panel.py) · [Validación](../../../../../../artifacts/rp4_code/evaluate.py) · [Inferencia registrada](../../../../../../artifacts/rp4_v3_a1_empty_window/specification.json) |
| Objetivo RV30 no anualizado y preguntas originales | v3 responde a RV30; v4 pasa a RV15 primario y RV5 secundario conservando estimador, claves y procedimiento. | **ALINEADO** para RV30; **DESVIACIÓN DECLARADA** de horizonte primario para la extensión, después de leer RV30. | [Objetivo](../../../../../../src/mds650/rp2/realized.py) · [Resultados RV30](../../../../../../artifacts/rp4_v3_b2/summary.json) · [Registro v4](../../../../../rp4/specification_v4.md) |
| Universo inicial: SPY, QQQ, AAPL, MSFT, NVDA, TSLA, AMZN y META | Objetivos: AAPL, AMZN, META, MSFT, NVDA y TSLA; SPY/QQQ entran como contexto de mercado en B0. | **DESVIACIÓN DECLARADA**: se pasa de ocho objetivos propuestos a seis. El papel está fijado en la especificación; **no se localizó una razón escrita** de la reducción en las decisiones y especificaciones inspeccionadas. No se presume que fuera por cobertura, liquidez ni resultado. | [Alcance original](../correction_4/proposal_source_extract.json) · [Universo fijado](../../../../../rp4/specification_v1.md) · [Decisiones](../../../../../methodology_decisions.md) · [Alcance de búsqueda](../correction_4/proposal_alignment_evidence.json) |
| Comparación de modelos: persistencia estacional, HAR-RV de Corsi, lineal regularizado y árboles | Lineal winsorizado con filtro de rango —ridge nominal— y LightGBM; HAR/HARQ y estacionalidad forman parte de B0. | **DESVIACIÓN DECLARADA** en el conjunto comparado; **PENDIENTE** la comparación separada HAR-RV/persistencia estacional. Tener esos predictores no entrega aquellas filas de benchmark ni convierte una ridge casi sin penalización en regularización sustantiva. | [Modelos comprometidos](../correction_4/proposal_source_extract.json) · [Implementación](../../../../../../artifacts/rp4_v3_code/models.py) · [Tabla de versiones](../../../../../../artifacts/rp4_closeout_figures/comparison_v1_v4.csv) |
| Evaluación secundaria MAE y RMSE | No figuran en los resultados RP4 inspeccionados; hay funciones compartidas que podrían calcularlas. | **PENDIENTE**, sin ejecución en esta revisión. La ausencia se refiere al flujo y a las salidas RP4 examinados, no a todo el repositorio. | [Compromiso](../correction_4/proposal_source_extract.json) · [Métricas compartidas](../../../../../../src/mds650/metrics.py) · [Tabla primaria](../../../../../../artifacts/rp4_v4_b4/primary_statistics.csv) |
| Sensibilidades temporales conservadoras | Se aplica un único proxy de tiempo fuente a **120 segundos**, sin resultados de variantes del corte en las salidas inspeccionadas. | **PENDIENTE** la sensibilidad PIT; un corte fijo no satisface una comparación de cortes ni prueba disponibilidad al cliente. | [Registro](../../../../../../artifacts/rp4_v4_a1/specification.json) · [Inventario acotado](../correction_4/proposal_alignment_evidence.json) |
| Pruebas placebo | No se encontró una salida de placebo en el flujo y los agregados RP4 inspeccionados. | **PENDIENTE**, sin ejecución en esta revisión; no se sustituye por diagnósticos de cobertura o convergencia. | [Compromiso](../correction_4/proposal_source_extract.json) · [Inventario acotado](../correction_4/proposal_alignment_evidence.json) |
| Análisis por activo y régimen | Existen secundarios por activo, hora, calendario y proxies de actividad/exposición, además de perfiles cronológicos descriptivos. | **ALINEADO** en los desgloses entregados; RQ3 queda **parcial**, sin afirmar estabilidad bajo todos los regímenes de volatilidad y supuestos temporales originales. | [RV30](../../../../../../artifacts/rp4_v3_b2/summary.json) · [Robustez v4](../../../../../../artifacts/rp4_v4_b4/robustness.csv) · [Secundarios v4](../../../../../../artifacts/rp4_v4_b4/regime_secondary.csv) |

### El compromiso de aceptar resultados adversos

La propuesta española dice literalmente: **«La elección del modelo dependerá del benchmark y de la simplicidad, no de que B2 produzca un signo favorable.»** La versión inglesa dice: **“Model choice will follow benchmark performance and simplicity, not whether B2 produces a favourable sign.”** El [extracto original](../correction_4/proposal_source_extract.json) conserva ambas frases. Dentro de cada familia, el [selector ejecutado](../../../../../../artifacts/rp4_v3_code/models.py) usa QLIKE de validación y desempates por mayor penalización o menos hojas/rondas. Esto respalda el principio dentro del ajuste, sin borrar la adaptación entre versiones.

Junto a ese compromiso, la [pre-declaración v4](../../../../../rp4/predeclaration_v4_text.md) fija literalmente: **«Si no: el proyecto se cierra con B1 > B0 como resultado principal y B2 como nulo informativo a 30, 15 y 5 minutos. No hay v5.»** Se conserva también la precisión de la [especificación ejecutada](../../../../../rp4/specification_v4.md): no rechazo no establece equivalencia a cero ni absorción causal. Ambos documentos expresan disposición a publicar un resultado adverso, pero **no son la misma regla inferencial**: v3 requería ambas familias para la conclusión conjunta y v4 permite al menos una, con RV5 secundario y sin control global de la búsqueda entre versiones. La propuesta no pre-especificaba esa regla de éxito. Se declara esta diferencia en lugar de presentar el cierre como prueba de ausencia de selección histórica.

Al mostrar el no rechazo del flujo a RV30 y el no rechazo de la media de árboles se conserva la frase exacta española **«un resultado nulo o negativo sería igualmente valioso»**, y la inglesa **“a null or negative result would be equally informative”**. El [apartado de resultados esperados](../correction_4/proposal_source_extract.json) las vincula a la contribución de un protocolo verificable; aquí «nulo informativo» describe un resultado no detectado, no una demostración de inutilidad del flujo. Las formulaciones «seguirá el rendimiento» e «igualmente informativo» son paráfrasis españolas del encargo; las citas anteriores reproducen las copias originales inspeccionadas.

### Los diez entregables comprometidos

**ALINEADO** significa que existe una entrega con esa función, no que se haya aprobado institucionalmente ni que todos sus subcompromisos estén cumplidos. **DESVIACIÓN DECLARADA** identifica una diferencia documentada de alcance o acceso; **PENDIENTE** incluye archivos existentes que aún describen el diseño anterior. La lista sigue el apartado de resultados esperados de la [propuesta](../correction_4/proposal_source_extract.json).

| Entregable comprometido | Estado | Ruta y alcance comprobado |
| --- | --- | --- |
| Informe final | **ALINEADO** | [RESULTADO_FINAL_revision_2.md](../../../../../rp4/RESULTADO_FINAL_revision_2.md) y [resumen bilingüe](executive_summary.md): pregunta, diseño, versiones, estimación, incertidumbre y límites; no acredita aceptación del formato institucional. |
| Presentación | **ALINEADO** | [defense_slides.md](defense_slides.md) y [manifiesto de figuras](../../../../../../artifacts/rp4_closeout_figures_revision2/manifest.json): guion Markdown con notas y gráficos; no se afirma una entrega nativa de diapositivas ni una duración validada. |
| Matriz de literatura | **ALINEADO**, con subcompromiso pendiente | [docs/literature_matrix.csv](../../../../../literature_matrix.csv) y [ledger de acceso](../../../../../literature_evidence_ledger_v2.csv): **10** estudios; una fila se identifica como FEDS working paper. La propuesta exige al menos diez revisados por pares; esa matriz por sí sola no acredita que sus filas satisfagan el requisito y esta revisión no amplía el censo editorial del proyecto. Texto completo, versión de trabajo y metadatos siguen diferenciados; no se amplía la revisión aquí. |
| Diccionario de datos | **PENDIENTE** de actualización RP4 | [docs/data_dictionary.md](../../../../../data_dictionary.md) existe, pero mantiene estados antiguos de barras/B1 bloqueados y objetivo RV30; el [registro de variables actual](../../../../../../artifacts/rp4_v4_a1/specification.json) no sustituye un diccionario de campos, unidades y transformaciones actualizado. |
| Código reproducible de extracción y calidad | **ALINEADO** en entrega | [Clientes](../../../../../../src/mds650/providers/fmp.py), [calidad](../../../../../../src/mds650/quality.py), [manifiestos](../../../../../../src/mds650/manifests.py), [evaluador](../../../../../../artifacts/rp4_v4_code/evaluate_v4.py), [lockfile](../../../../../../uv.lock) y [RUNBOOK.md](../../../../../rp4/OPERATING_GUIDE.md). Código y procedimiento disponibles; la reproducción completa desde los originales licenciados no se ejecutó en esta revisión. |
| Panel analítico versionado | **DESVIACIÓN DECLARADA** de acceso | [Especificación](../../../../../../artifacts/rp4_v4_a1/specification.json), [freeze](../../../../../../artifacts/rp4_v4_a1/freeze_manifest.json), [recibo](../../../../../../artifacts/rp4_v4_a2/receipt.json) y [release](../../../../../../artifacts/rp4_v4_a2/evaluation_release_rv15.json) fijan identidades y hashes. El payload permanece en custodia privada por licencia; aquí se verifican metadatos, sin abrirlo ni afirmar reconstrucción pública completa. |
| Tablas de comparación | **ALINEADO** | [comparison_v1_v4.csv](../../../../../../artifacts/rp4_closeout_figures/comparison_v1_v4.csv) y [primary_statistics.csv](../../../../../../artifacts/rp4_v4_b4/primary_statistics.csv): incluyen versiones, ventanas, familias, contrastes y signos adversos; los benchmarks separados pendientes no se dan por entregados. |
| Intervalos de incertidumbre | **ALINEADO** | [primary_statistics.csv](../../../../../../artifacts/rp4_v4_b4/primary_statistics.csv): intervalos por contraste y regla bootstrap. El intervalo porcentual usa denominador medio fijo, no bootstrap del cociente; no se regeneró. |
| Análisis de robustez | **ALINEADO** en los análisis presentes | [robustness.csv](../../../../../../artifacts/rp4_v4_b4/robustness.csv), [regime_secondary.csv](../../../../../../artifacts/rp4_v4_b4/regime_secondary.csv) e [informe de auditoría](../../../../../../artifacts/rp4_closeout_audit/REPORT.md). No dan por ejecutados MAE/RMSE, variantes PIT, placebo ni toda RQ3. |
| Cuaderno para el examinador | **PENDIENTE** de actualización RP4 | [canonical_rv30_defense.ipynb](../../../../../../notebooks/canonical_rv30_defense.ipynb) y [research_pipeline.ipynb](../../../../../../notebooks/research_pipeline.ipynb) existen, pero apuntan a evaluaciones anteriores; el segundo conserva B1 inviable y comparación alternativa. Sus celdas se inspeccionaron como texto, sin ejecución ni uso de outputs. Esta guía Markdown no se presenta como el cuaderno prometido. |

**Pendientes documentados, sin ejecución.** MAE/RMSE, variantes PIT, placebo y benchmarks separados HAR-RV/persistencia estacional quedan a decisión del propietario como robustez comprometida. También se documentan la actualización del diccionario/cuaderno y la acreditación del requisito bibliográfico; no se completan silenciosamente durante una auditoría de alineación. Esta revisión añade texto, tablas y contratos sobre evidencia guardada; no añade pérdidas, ajustes, intervalos ni datos y no cambia ningún registro prospectivo.
...
H1 lineal es marginal a RV30 (p 0.0439) y RV15 (0.0390); no había rechazo lineal B1/B0 en v1/v2. La sensibilidad cerrada con los p bilaterales guardados y Holm de cuatro contrastes deja sin rechazo a ambos H1 lineales; la tabla de la pregunta 26 conserva todos los horizontes y no mezcla familias para abrir H2.
...
## 26. Multiplicidad entre versiones: ¿qué sobrevive bajo la regla bilateral/Holm antigua?

La sensibilidad se cierra con los p **bilaterales guardados** y su Holm bilateral sobre cuatro contrastes dentro de cada horizonte de la ventana primaria. RV5 sigue siendo un objetivo secundario. La tabla sustituye las aproximaciones anteriores como respuesta a la regla antigua; los documentos anteriores permanecen sellados. No se duplica automáticamente el p unilateral: las colas de la distribución centrada pueden ser asimétricas.

| Horizonte | Familia / contraste | p unilateral registrado | p bilateral guardado | Holm bilateral sobre cuatro contrastes | Rechazo individual |
| --- | --- | ---: | ---: | ---: | --- |
| RV15 | Lineal B1/B0 | 0,0390 | 0,0459 | 0,0918 | No |
| RV15 | Lineal B2/B1 | 0,0032 | 0,0062 | 0,0248 | Sí |
| RV15 | Árboles B1/B0 | 0,0135 | 0,0149 | 0,0447 | Sí |
| RV15 | Árboles B2/B1 | 0,6280 | 0,8048 | 0,8048 | No |
| RV30 | Lineal B1/B0 | 0,0439 | 0,0467 | 0,1401 | No |
| RV30 | Lineal B2/B1 | 0,0525 | 0,1013 | 0,2026 | No |
| RV30 | Árboles B1/B0 | 0,0053 | 0,0066 | 0,0264 | Sí |
| RV30 | Árboles B2/B1 | 0,6631 | 0,7249 | 0,7249 | No |
| RV5 | Lineal B1/B0 | 0,0092 | 0,0099 | 0,0396 | Sí |
| RV5 | Lineal B2/B1 | 0,0172 | 0,0343 | 0,1029 | No |
| RV5 | Árboles B1/B0 | 0,0608 | 0,0731 | 0,1462 | No |
| RV5 | Árboles B2/B1 | 0,1927 (nominal) | 0,4014 | 0,4014 | No |

Bajo la sensibilidad a la regla bilateral/Holm de v1/v2, el flujo sobrevive sólo en la familia lineal a RV15 y el estado sobrevive en árboles a RV30 y RV15 y en la lineal a RV5. La secuencia lineal completa a RV15 no se habría abierto si se exigiera además H1 tras Holm, porque ese H1 no rechaza. V1/v2 no usaban H1→H2: la última frase es una interpretación condicional, no la atribución retrospectiva de una secuencia a esas versiones. Un rechazo H1 de árboles no abre H2 lineal.

El p unilateral de H2 árboles RV5 es **nominal**, con `p_for_decision` vacío, `hypothesis_status=NOT_TESTED` y `nominal_p_role=NO_PROMOTABLE`; su presencia en esta sensibilidad no reabre la prueba registrada. Cada celda conserva su columna e índice exactos del CSV en la matriz. Holm opera sobre cuatro contrastes por horizonte, no sobre una familia global que abarque horizontes, versiones y decisiones adaptativas. Esta lectura posterior tampoco hace independientes las muestras reutilizadas ni cambia las decisiones originales.

Fuentes: [p guardados](../../../../../../artifacts/rp4_v4_b4/primary_statistics.csv), [filas exactas y alcance](../correction_5/holm_evidence.json), [regla original v1](../../../../../rp4/specification_v1.md), [regla v2](../../../../../rp4/specification_v2.md), [secuencia v3](../../../../../rp4/specification_v3.md) y [cierre v4](../../../../../rp4/specification_v4.md).
...
## 28. ¿Qué quedó de los secundarios que fallaron y qué significa que estén reparados?

El endpoint de clasificación de saltos tiene AUC final calculable en las dos ventanas y ambas familias. Ningún H1 rechaza; cada H2 conserva `NOT_TESTED` y su p nominal no sirve para decidir. Reparar la convergencia permite informar el contraste registrado, pero no convierte un resultado adverso en evidencia favorable ni demuestra ausencia de información.

La secuencia de intentos se conserva: el primer pase de reparación dejó un componente lineal sin certificado; Newton resolvió después sólo ese componente fallido, reutilizando los ya completos. Cambió el procedimiento numérico del intento reparado, no la función objetivo, la selección temporal, la muestra registrada ni el umbral. El certificado más estricto de las reparaciones no se atribuye retroactivamente a todos los ajustes originales.

En Mincer–Zarnowitz hay que separar el diagnóstico de calibración de pronósticos originales, disponible para ambas familias, de la recalibración afín aplicada, que corresponde únicamente a LightGBM. Los recibos guardados reproducen la fórmula por origen y los agregados dentro de precisión numérica. Las salidas afines negativas, llevadas al piso positivo registrado, generan pérdidas QLIKE muy grandes; la identidad aritmética no demuestra una recalibración útil. Se conserva el cierre de utilidad `NO VERIFICABLE`, con su causa numérica y estadística explícita, sin presentarlo como imposibilidad de reproducir la cuenta.

Entre los endpoints AUC y MZ aquí examinados, no se identifica un agregado final que siga siendo irreproducible en los recibos guardados. Esta revisión coteja agregados y procedencia guardados; no reejecuta modelos ni vuelve a leer predicciones individuales. Los resultados revisados y el resultado final ya incluían estos secundarios; la nueva edición añade una proyección pública acotada y su explicación, preservando los intentos fallidos y las tablas previas.

**Evidencia.** [Secundarios reparados de v3](../../../../../rp4/results_v3_revision3.md) · [Proyección verificable](../../../../../../artifacts/rp4_v3_secondary_repair/evidence.json) · [Revisión anterior](../../../../../rp4/results_v3_revision2.md) · [Resultado final conservado](../../../../../rp4/RESULTADO_FINAL_revision_2.md).
