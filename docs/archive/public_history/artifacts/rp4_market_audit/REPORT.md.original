...
El corte alternativo del 29 de enero y los desgloses por activo, junto con una ventana local de ±28 días, permanecen en [la tabla de cobertura completa](coverage_weekday.csv). Son comparaciones descriptivas; no identifican por sí mismas una causa del rendimiento predictivo.
...
[Fuentes: alias y hashes](sources.json) · [Recibo de ejecución](receipt.json) · [Proyección pública](publication_projection.json). Los hashes identifican entradas sin exponer rutas privadas; los registros por minuto se excluyen de esta proyección.
