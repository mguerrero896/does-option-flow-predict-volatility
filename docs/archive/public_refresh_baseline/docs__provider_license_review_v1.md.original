...
| Not stored in either | Raw provider payloads (1-min bars, tape archives, quote sweeps) — live only on the owner's local `<DATA_ROOT>` | — |
...
support@unusualwhales.com, Massive support. No consent request is represented as sent
by this review; provider outreach requires a separately authorized communication.
