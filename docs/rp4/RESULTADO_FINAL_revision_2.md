...
This describes the training-window evaluation; the design was fixed after the sample had already been observed.

# Do options improve intraday volatility forecasts?

Yes in the development sample for option-state information; for flow, the answer depends on the horizon, family and statistic. The RP4 closeout rule was met at RV15 in the linear family. The scientific programme ends at v4, without a v5.
...
A linear family and LightGBM are compared. In v3/v4 the linear family is a **winsorised linear model with a rank filter (nominal ridge)**. QLIKE loss measures forecast error, not profitability. In v3/v4, H1 tests B1 over B0 at one-sided 5%; only if it rejects is H2, B2 over B1, tested, also at 5%. The v4 closeout rule requires the sequence in at least one family, not both. The bootstrap uses five-session blocks; the p-values do not correct the search across versions.
...
The dates 2025-05-15 and 2025-09-18 are flagged as measurement-instrument incidents; the census establishes empty windows without independently verifying a publication interruption. The v3 rule retains zero activity, makes shapes/ratios undefined when there are no trades and adds indicators; it bounds the harm without eliminating it. There remain 412 empty five-minute windows in the primary sample and none in confirmation. They are not excluded to improve the result. In those rows, linear QLIKE rises from 0.103 in B1 to 0.202 in B2 with equal session/asset weights; the repair limits the harm rather than eliminating it.
...
This is the fourth evaluation of the same windows: (a) v1/v2 used two-sided p-values with Holm over four contrasts, whereas v3/v4 used a sequential one-sided test at 5% per family without Holm across families, a change registered before v3 that makes the decision less demanding; (b) v3 expanded B2 with gamma imbalance and its indicators after the failures of v1/v2; (c) decision 132 fixed the empty-window rule after the v2 diagnostic; (d) v4 changed the horizon through a predeclared, locally sealed conditional rule, without a third-party timestamp; (e) the split was fixed on 2026-09-07 after the entire sample had already been observed, so RV5 is secondary and the search across versions is uncorrected.

The earlier read of 2026-07-20–2026-08-28, under another specification, overlaps 20 of the 25 final sessions; the historical record reports a mixed exploratory, non-confirmatory result and a favourable sensitivity in one of eight primary cells that included B1, so those sessions are not presented as unknown when the split was fixed.

PIT uses a source-time proxy at 120 s and does not prove historical availability to the client.

The provider gap 2025-01-25–2025-02-24 is retained without filling.

The [v3 declaration](specification_v3.md), [empty-window decision](decision_132_v3_empty_windows.md), [projected text of the conditional antecedent](predeclaration_v4_text.md) and [evidence chronology and limits](DEFENSE_PACKAGE/revision_2/correction_2/examiner_qa.md) document this sequence; the overlap is calculated exclusively from the already saved dates of the final window, and the earlier read is taken from the historical disclosure, not from a new inspection of that study.

## Sensitivity and scope of the conclusion

The one-sided p-values for linear H1, 0.0439 at RV30 and 0.0390 at RV15, are marginal and do not survive the Bonferroni sensitivity across the RV30/RV15 horizons; linear H1 also did not reject in v1/v2. Applying Holm to the four **saved** two-sided RV15 p-values gives adjusted p-values of 0.0918 for linear B1/B0, 0.0248 for linear B2/B1, 0.0447 for tree B1/B0 and 0.8048 for tree B2/B1: linear B2/B1 and tree B1/B0 survive, but neither family passes both steps. This comparison uses the absolute tails of the saved two-sided bootstrap; automatically doubling a one-sided p-value does not reproduce that test. No resampling is repeated and the registered v4 decision is unchanged. [Retained statistics](../../artifacts/rp4_v4_b4/primary_statistics.csv).

Sign agreement across the six assets at RV15 represents consistency within a homogeneous sample of large technology or technology-related companies, not six independent replications; resampling uses the session and does not establish generalisation to other assets. No new correlation matrix has been measured here. RV5 results, positive for only three assets, are retained separately.

The [prospective amendment 3](prospective_confirmation_v1_amendment_3.md) retains the 20, 40 and 45 rules as consistency checks and registers a final cumulative read at 335 new eligible sessions, without rescue or additional interim inspections: the approximate normal power for H2 is 80.05%, but H1 reaches 54.98%, an upper bound on the full sequence under those approximations; no global 5% control across reads is claimed.

RESEARCH_ONLY · NOT INVESTMENT ADVICE · capital_go=false.

## Documentary revision note

This revision corrects the scope of the conclusion, discloses the design over the observed sample, distinguishes results by horizon and extends disclosures and sensitivities using already saved evidence. It retains the full results table and all original scientific artifacts; the power figures are analytical planning calculations, not new v3/v4 results. The [previous revision](RESULTADO_FINAL_revision_1.md) retains its clarification of premium proportions and its own seal. The [receipt for this revision](RESULTADO_FINAL_revision_2_receipt.json) distinguishes both documents and their hashes.
