...
**Point-in-time sensitivity (300 seconds, completed):** with the option-information cutoff moved from 120 to 300 seconds, the option-state increment remains supported in this sensitivity (**0.912%**, nominal p = **0.0342**), but the linear flow increment falls to **0.194%** (nominal p = **0.1948**; 95% interval **[-0.000445; 0.001150]** crosses zero). The historical flow gain depends on the 120-second availability assumption; stability across conservative timing assumptions is not established. This does not demonstrate zero flow information or change the registered primary decision. The saved 120-second control reproduces the primary contrasts exactly; both cutoffs cover 419 sessions and 160,832 origins. **60-second cutoff: not yet completed.** [Saved timing contrasts](../artifacts/rp4_robustness_public_v1/pit_300_contrasts.csv) · [Import provenance](../artifacts/rp4_robustness_public_v1/import_receipt.json).
