...
These are nested information sets, not necessarily independent economic feeds. The valid incremental interpretation is information beyond the implemented B1 representation. “Flow” below abbreviates the complete 69-column increment, not informed trading or dealer hedging in isolation. [B2 interpretation, concentration and missing controls](B2_INTERPRETATION.md). The primary target is RV15 (15-minute realized variance); RV5 is secondary. The families are winsorized log-variance ridge regression with rank filtering and LightGBM.
...
**Placebo (RV30, completed):** the observed linear B2/B1 increment is **+0.0008024** QLIKE, versus a shuffled-flow mean of **0.0012107** (**150.9%** of the observed); **38 of 50** permutations exceed the observed, with empirical p = **39/51 = 0.7647**. The joint 69-column flow block is shuffled within each asset-session, covering **419 sessions** and **20,950 fits** across 50 permutations. At 30 minutes, shuffled flow performs better on average than aligned flow, so this control does not support a minute-by-minute alignment advantage. This is consistent with the registered RV30 second test not rejecting in v3 (**p = 0.0525**), not a causal decomposition or a change to the RV15 headline. The whole-session shuffle is not an executable real-time information set. [Saved RV30 placebo summary](../artifacts/rp4_robustness_public_v1/placebo_log_ridge_harq_rv30_summary.csv) · [Registered RV30 result](rp4/results_v3.md) · [Import provenance](../artifacts/rp4_robustness_public_v1/import_receipt.json).
...
**Code lineage:** the frozen RP4 v4 aggregates and headline were produced by the B2 producer as it existed before the history-eligibility repair published on 13 September 2026 ([D89](known_defects_and_resolutions.md)). The current producer applies availability exclusions before normalization, keeps excluded or incomplete features null and requires 20 eligible prior sessions with 80% eligible-origin coverage. Re-running the panel builder with the current code therefore changes B2 features and would not reproduce the frozen aggregates byte-for-byte; those aggregates have not been recomputed. The repair's effect was measured on earlier campaigns, not on the 419-session v4 panel: 27,153 of 77,328 origins changed features in the 180-session B2 universe (244,377 cells) and 24,604 in the 159-session published panel (221,436 cells); in an isolated re-evaluation the B2-over-B1 QLIKE gain moved from -0.003127 to -0.002206 for the Gamma model and from +0.001368 to +0.001055 for LightGBM, with B1-over-B0 unchanged. [Closeout note](b2_repair_and_evidence_closeout_20260905.md) · [Lineage record with producer hashes](../data/b2_producer_lineage.json).
...
Amendment 4 records the September collector incident and owner-authorised late acquisition of 8–10 September, sealed before acquisition and any replication read. The [subsequent execution receipt](rp4/prospective_confirmation_v1_amendment_4_execution_receipt.json) records collection **PASS for 8 and 9 September**, and **UNAVAILABLE_LATE for 10 September**: AAPL contained 389 of 390 expected minutes. Each attempt passed the 105 GiB pre-session gate; the final observed free space was 121.57 GiB. The incomplete session is excluded without imputation, substitution or replay. The complete sessions enter the chronological cohort only under unchanged eligibility rules; collection PASS is not a prospective result. The original seal retains its pending-at-seal state; source-time limits and decision rules remain unchanged.
