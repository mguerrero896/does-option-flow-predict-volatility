...
The `pit_300` section of the [import receipt](../artifacts/rp4_robustness_public_v1/import_receipt.json) records `COMPLETE_CONTRACT_PASS`, original/public hashes for five outputs and the declared JSON transformations. The three CSVs are byte-identical. The [session losses](../artifacts/rp4_robustness_public_v1/pit_300_session_losses.csv) reproduce the two 300-second means, percentage denominators and 217 positive flow sessions; the two 120-second rows in the contrasts match the saved primary estimates, intervals, p-values and counts exactly. The [availability table](../artifacts/rp4_robustness_public_v1/pit_300_availability.csv) contains column-level counts, not granular provider observations. Source-time proxy sensitivity is not historical client-receipt proof; **60-second cutoff: not yet completed**.
