...
> **Current status — 2026-08-28.** This page retains the implementation history of the
> versioned publication design. It is not an active publication instruction. Migration
> `20260827073145_retire_ineligible_current_rp2_results` retired every current result and
> revoked `public.publish_rp2_v3(jsonb)` from `service_role`; the four base tables and four
> current views each contain zero current rows. The live read-only measurement is
> `artifacts/supabase_schema_audit_20260828.json`. Migration
> `20260828020327_close_service_role_sensitive_mutation_paths` also revoked direct
> service-role mutation privileges on the retired result tables without changing the six
> dataset-loader tables. Publishing remains disabled until a future eligible run and a
> separately authorized migration restore a reviewed publication path.
...
## Loader and catalog preflight

A read-only PostgREST OpenAPI measurement on 2026-08-26 is recorded in
`artifacts/supabase_schema_preflight_20260826.json`. All six staging tables exist, but none
has the run-scoping `load_id` column. The legacy `promote_dataset` RPC exists;
`promote_dataset_v2` and `reconcile_research_catalog` do not. No table rows or sealed data
were read and no write was attempted.

The owner applied that migration as
`supabase/migrations/20260826180409_loader_run_scoped_staging.sql`. The schema and ACL
checks passed: all six `load_id` columns exist, all seven private tables have RLS, and only
`service_role` can execute the two new functions. The first catalog synchronization then
failed closed with SQLSTATE `21000`, `DELETE requires a WHERE clause`: this project's
`safeupdate` guard rejects the two intentional whole-table reconciliation deletes and the
live-table replacement in the promotion function when they omit an explicit predicate.

The owner then applied the minimal correction as
`supabase/migrations/20260826194803_safeupdate_compatible_reconciliation.sql`. It adds
`WHERE true` to those three deliberate replacements and revokes `service_role` from the
superseded, non-allowlisted `promote_dataset` RPC. The catalog reconciliation committed
5 campaigns, 36 contrasts, 264 MCS cells and 15 gated-file records.

The loader promoted all six registered datasets and a subsequent full rerun skipped all six
by exact SHA-256. The two largest promotions exceeded the REST role's two-minute statement
timeout; their transactions rolled back, and the same staged load IDs were retried with a
session-local ten-minute timeout. Final staging rows were zero. One b1v3 retry initially
recorded an incorrect digest; a value-conditioned metadata correction restored the verified
file SHA-256 before the all-six idempotence check. Throughout the sequence,
`sealed_cohorts_read=0`.

## What the migration adds

`supabase/migrations/20260821034446_rp2_v3_versioned_results.sql`:
...
- `api.current_rp2_block_results`, `api.current_rp2_contrasts`,
  `api.current_rp2_extension_results` and `api.current_rp2_power_results`. All four are
  `security_invoker`; migration `20260826210228_close_versioned_result_base_tables.sql`
  revokes anonymous/authenticated access to the views and their base tables. That migration
  initially retained the service-role publication path; migration `20260827073145` later
  disabled it and retired all current results;
...
## Historical transactional publication design (currently disabled)

When the RPC was enabled, `scripts/publish_rp2_v3_supabase.py` made a single call to
`public.publish_rp2_v3`, because
...
At the time of this historical dry-run record, production afterwards was `blocks null`,
`power null`, `contrast_view null`, `runs 1`, `new_cols 0`; the migration had not yet been
applied. The current live state is governed by the status banner above.
