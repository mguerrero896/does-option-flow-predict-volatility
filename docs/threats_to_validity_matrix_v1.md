...
(`sealed_cohorts_read=0`). D/V entries below are historical diagnostics classified as
`HISTORICAL_MEASUREMENT_NOT_CURRENT_CLAIM`, not current results.
...
| 5 | Serial dependence and interval calibration | Five-minute origins overlap; the 0.784 empirical coverage measurement against nominal 0.95 belongs to the superseded RP2 run and has not been repeated on the current run | Aggregate to sessions; block bootstrap, Newey-West and wild-cluster diagnostics | Intervals may still undercover, especially in short V |
