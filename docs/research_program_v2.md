...
**Status:** `HISTORICAL_DESIGN_RECORD — EXECUTED_AND_SUPERSEDED`
**Recorded:** 2026-08-18
**Authority:** This document preserves the design that produced the historical RP2-v2
cascade. It is not an instruction to the repository maintainer or to automation, and it
does not define the current result. Current authority is `data/CANONICAL_STATE.json`; the
current methodological interpretation is `docs/rp2_v3/VERDICT.md`, with withdrawn claims
tracked in `docs/rp2_v3/SUPERSEDED_RESULTS.md`.
...
*Historical execution notes retained to explain the ordering and constraints used by the
2026-08-18 cascade. They are not current operating instructions.*
