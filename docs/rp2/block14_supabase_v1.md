...
> **Historical audit record — superseded.** This document records the schema observed and
> decisions made on 2026-08-18. It is not the current access or migration authority. Use
> `supabase/README.md`, `data/DATA_ACCESS.md` and
> `artifacts/supabase_schema_audit_20260828.json` for the reconciled live state: 19 applied
> migrations, `security_invoker` API views, six hash/count-bound dataset loads, and zero
> current RP2 results. The pending Block 14 draft remains explicitly unsafe to run as written.
...
**Withheld, pending documented approval:** `supabase/migrations_pending/rp2_block14_pending.sql`
...
requiring the documented approval.
