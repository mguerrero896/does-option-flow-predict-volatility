...
SOURCE_HASH_MODE = {
    "default": "raw-bytes",
    "by_suffix": {".py": "utf8-text-crlf-to-lf", ".cjs": "utf8-text-crlf-to-lf"},
}
...
def workflow_variant() -> bytes:
    root = ET.fromstring((ROOT / WORKFLOW).read_bytes())
    replacements = {
        "RP4 v4 · RV15": "15-minute study",
        "419 sessions · walk-forward": "419 sessions · time order",
        "Earlier programmes": "Prior studies",
        "Nulls, mixed, not confirmed": "Earlier mixed results",
        "SEALED": "Registered",
        "PENDING": "Future",
        "A workflow diagram generated by Archify.": (
            "Study design, evaluation and prospective replication."
        ),
    }
    for node in root.iter():
        ...
        for key in ("aria-label", "title"):
            if key in node.attrib:
                for old, new in replacements.items():
                    node.attrib[key] = node.attrib[key].replace(old, new)
...
def rasterization_svg(data: bytes) -> bytes:
...
def main() -> int:
    ...
    outputs = {
        ...
        HERE / "proposal_to_replication_readme.workflow.svg": workflow_variant(),
    }
    svg_sources = outputs | {
        ROOT / p: (ROOT / p).read_bytes() for p in (WORKFLOW, TIMELINE, GLOBAL)
    }
    ...
    for path in (*outputs, ROOT / GLOBAL):
        visible_text = " ".join(
            (node.text or "")
    ...
    if args.render:
        ...
        manifest = {
            ...
            "method": (
                ...
                "original figure style and Sharp rasterizer. The README workflow changes "
                "labels and accessibility text only in the existing Archify export. "
