...
126. **The PR #55 panel-reuse record is corrected additively: source admission is
     ...
     | Inventory binding | **THE HISTORICAL REUSE BRANCH OMITTED `assert_inventory_is_frozen`; BOTH BRANCHES NOW REQUIRE IT, AND A SOURCE MUST RECORD THAT SAME INVENTORY.** The default rebuild already bound `inventory.jsonl` to the partition before session or tape use. In the opt-in branch, the root sealed-path guard runs first, `is_dir()` then verifies the source shape, and the inventory comparison still precedes manifest interpretation, `assert_partition_matches` and `assert_tape_covers_panel`. After the source input manifest is hash-verified and loaded, its `tape_inventory_sha256` must equal `normalised_digest(TAPE_INVENTORY)` or admission stops with `RP2_RUN_PANEL_SOURCE_INVENTORY_MISMATCH`. A regression fixture supplies a self-consistent registered bundle with a different inventory hash and proves the fail-closed boundary. |
     ...
     | Tier 2 | **SEVEN OF SEVEN GATES PASSED WITH THE LICENSED OPT-OUTS ABSENT ON THE CURRENT MAIN-LINE IMPLEMENTATION TREE.** `scripts/run_local_evidence_gates.py` constructs an explicit inherited environment after removing `MDS650_PANEL_GUARD_MAY_SKIP` and `MDS650_UW_LATENCY_FRESHNESS_MAY_SKIP` case-insensitively for every licensed gate; it now also creates receipts exclusively and refuses overwrite. Only the hermetic CI replica declares both opt-outs as `1`. At `2026-09-01T21:25:03.366835+00:00`, public implementation commit `869ab824646d500dd1a288f910b8f624f38bf868` (tree `471cf3ad4ac266358df71dcea85d4d43e5e7d998`) returned code zero for `versioned-hook`, `ruff`, `mypy`, `full-pytest`, `ci-sim`, `gated-hashes` and `access-posture`; then-current `main` commit `8d79c9b09e3f05b976089b10bd3c5b9f936768cc` is the required ancestor. The current immutable receipt is `artifacts/local_evidence_gates/pr55_remediation_20260902_v6.json`, canonical LF SHA-256 `c85cadd69ef8c6406d3d5d53582cae7a222d255ba4d5d4a30e392d9ae7c21754`. Receipts v1/v2 are superseded local records whose tested commits were never public; v3 is an earlier rebased receipt. Local v4/v5 runs were retained outside Git as superseded diagnostics after, respectively, main advanced and the required-ancestor target was corrected. A receipt cannot include the hash of the commit that first adds itself; the sole post-test overlay is therefore restricted by contract to this receipt, its registry/generator/canonical outputs, this decision, STATUS and the overlay contract itself, and hosted PR checks cover that overlay. |
