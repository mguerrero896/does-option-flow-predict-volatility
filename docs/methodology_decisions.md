...
126. **The PR #55 panel-reuse record is corrected additively: source admission is
     ...
     | Tier 2 | **SEVEN OF SEVEN GATES PASSED WITH THE LICENSED OPT-OUTS ABSENT.** `scripts/run_local_evidence_gates.py` constructs an explicit inherited environment after removing `MDS650_PANEL_GUARD_MAY_SKIP` and `MDS650_UW_LATENCY_FRESHNESS_MAY_SKIP` case-insensitively for every licensed gate. Only the hermetic CI replica declares both as `1`. At `2026-09-01T20:12:33.122338+00:00`, rebased implementation commit `577fb5a9ba4bde494b5ac3a4270ad389f182df88` (tree `216c2a2eb5f9d19ba4cfa54f29d4147955d41267`) returned code zero for `versioned-hook`, `ruff`, `mypy`, `full-pytest`, `ci-sim`, `gated-hashes` and `access-posture`; `c1e331e247d411eb21141b1cd271130ae16bfd89` is a required ancestor. The current immutable JSON is `artifacts/local_evidence_gates/pr55_remediation_20260902_v3.json`, canonical LF SHA-256 `109f8c863fd2b3246dfa1b7b94861a16941483e0b697c6208a0b0eccbd35ed6e`; frozen `v1` and `v2` remain as earlier evidence instead of being overwritten. A prior clean-tree run was RED at 5/7, exposing the stale UW inventory and the now-landed PR #62 `ProgramFiles` omission before this GREEN run. |
     | UW freshness needed by Tier 2 | **NO FROZEN SNAPSHOT WAS OVERWRITTEN.** The first 2026-09-02 snapshot recorded 12 collected, six reconciled and six unreconciled sessions; v2 then recorded the 2026-09-01 collector summary. A later clean-tree Tier 2 run was RED because only `capture_report_present` for 2026-09-01 changed from false to true. Counts, reconciliation status and every latency aggregate remained unchanged. Current authority therefore advances additively to `uw_latency_campaign_20260902_v3.json` (file SHA-256 `c234b7cf0f13a177b398955b282124da1a4f69d35da64481f64c8037b10eb6f4`, byte-identical to v1/v2) and `uw_latency_campaign_state_20260902_v3.json` (file SHA-256 `29448ec0188c1a2ab4a6fcb9904c9a6f838e3925826e0f351d250ab42785dea9`, semantic SHA-256 `b08abea8e0e1c7aa39657476650a221498edcb56f132efb1dfa0a390e64a5676`). Every earlier pair remains frozen. No provider row is versioned and `sealed_cohort_read=false`. |
