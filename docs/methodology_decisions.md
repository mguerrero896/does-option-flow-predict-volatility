...
126. **The PR #55 panel-reuse record is corrected additively: source admission is
     ...
     | Tier 2 | **SEVEN OF SEVEN GATES PASSED WITH THE LICENSED OPT-OUTS ABSENT AFTER BOTH SOURCE-INPUT GUARDS.** `scripts/run_local_evidence_gates.py` constructs an explicit inherited environment after removing `MDS650_PANEL_GUARD_MAY_SKIP` and `MDS650_UW_LATENCY_FRESHNESS_MAY_SKIP` case-insensitively for every licensed gate; it also creates receipts exclusively and refuses overwrite. Only the hermetic CI replica declares both opt-outs as `1`. At `2026-09-01T22:17:26.448087+00:00`, public implementation commit `2c1aecb4058aec6bf442a46fab0c1b2286d7843f` (tree `d2a382ce01956684d418625c568a6eb733a706f9`) returned code zero for `versioned-hook`, `ruff`, `mypy`, `full-pytest`, `ci-sim`, `gated-hashes` and `access-posture`; merged PR #68 commit `e951a6b30961588bcc51066505b5604944120e14` is the required ancestor. The current immutable receipt is `artifacts/local_evidence_gates/pr55_remediation_20260902_v8.json`, canonical LF SHA-256 `7001dca86d857a888471f4f260d9688ecfc54f019ab9de0135664abe0e9db24f`. Receipts v6 and v7 remain immutable but are superseded because their tested trees preceded, respectively, the source-inventory and source-tape-fingerprint P1 corrections; v1/v2 are superseded local records whose tested commits were never public, v3 is an earlier rebased receipt, and local v4/v5 runs remain outside Git as superseded diagnostics. A receipt cannot include the hash of the commit that first adds itself; the sole post-test overlay is therefore restricted by contract to this receipt, its registry/generator/canonical outputs, this decision, STATUS and the overlay contract itself, and hosted PR checks cover that overlay. |
