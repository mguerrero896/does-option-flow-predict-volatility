...
126. **The PR #55 panel-reuse record is corrected additively: source admission is
     ...
     based on `8d79c9b0`, which contains the owner-required `c1e331e2` successor freeze,
     the PIT v2.2 successor closeout in PR #63 and the UW maturity guard in PR #66.

     | Boundary | Corrected disposition and evidence |
     | --- | --- |
     | Source identity | **THE SOURCE MANIFEST'S OWN SCIENTIFIC HASH IS RECOMPUTED; THIS IS NOT EXTERNAL AUTHENTICATION.** `assert_manifest_identity_intact` verifies the unkeyed scientific hash declared by the supplied bundle itself. There is no repository-anchored signature or independent digest for every claim in an arbitrary caller-supplied source. The tape content identity is separately constrained by a versioned canonical fingerprint, but that equality does not authenticate the rest of a caller-supplied bundle. Admission also verifies the registered step inventory, D/V roles, input-manifest identity, eight panel artifacts and their stable-content hashes. The first operation remains `assert_no_sealed_paths([source_run])`, before `is_dir()`, manifest access or any child read; `_source_artifact` repeats containment before file access. A mutation test that removes the root call fails on the first observed event, so the test fixes order rather than merely searching for the function name. |
     | Inventory and tape binding | **THE HISTORICAL REUSE BRANCH OMITTED `assert_inventory_is_frozen`; BOTH BRANCHES NOW REQUIRE IT, AND A SOURCE MUST MATCH BOTH THE FROZEN INVENTORY AND CANONICAL TAPE CONTENT IDENTITY.** The default rebuild already bound `inventory.jsonl` to the partition before session or tape use. In the opt-in branch, the root sealed-path guard runs first, `is_dir()` then verifies the source shape, and the inventory comparison still precedes manifest interpretation, `assert_partition_matches` and `assert_tape_covers_panel`. After the source input manifest is hash-verified and loaded, its `tape_inventory_sha256` must equal `normalised_digest(TAPE_INVENTORY)` or admission stops with `RP2_RUN_PANEL_SOURCE_INVENTORY_MISMATCH`. Inventory rows contain structure and sizes, not tape bytes, so the source must also record `tape_fingerprint_mode=content` and the canonical fingerprint `0a54fcea4801f8592f8b5c92aa8cbdf9cd97ad328ae8e5975c0c6f86b6710d3a`; a different, fast-mode or absent value stops with `RP2_RUN_PANEL_SOURCE_TAPE_FINGERPRINT_MISMATCH`. Regression fixtures supply internally self-consistent registered bundles that violate each boundary. No raw tape is opened by these comparisons. |
     ...
     | Tier 2 | **SEVEN OF SEVEN GATES PASSED WITH THE LICENSED OPT-OUTS ABSENT AFTER THE SOURCE-INVENTORY GUARD.** `scripts/run_local_evidence_gates.py` constructs an explicit inherited environment after removing `MDS650_PANEL_GUARD_MAY_SKIP` and `MDS650_UW_LATENCY_FRESHNESS_MAY_SKIP` case-insensitively for every licensed gate; it also creates receipts exclusively and refuses overwrite. Only the hermetic CI replica declares both opt-outs as `1`. At `2026-09-01T21:58:03.740688+00:00`, public implementation commit `e2ca6f67c6c00f7742c0b24a182873ef9c1b1fa4` (tree `2a32ead3c4fb067621ed58d61ed24346c7b03cb2`) returned code zero for `versioned-hook`, `ruff`, `mypy`, `full-pytest`, `ci-sim`, `gated-hashes` and `access-posture`; merged PR #67 commit `7ceec586636a65ce4d49290408885fb294ff0639` is the required ancestor. The current immutable receipt is `artifacts/local_evidence_gates/pr55_remediation_20260902_v7.json`, canonical LF SHA-256 `7da74938f4110fed8136b6b14f1270de1656dc8e408a1715fba0b1c0492965e2`. Receipt v6 remains immutable but is superseded because its tested tree preceded the source-inventory P1 correction; v1/v2 are superseded local records whose tested commits were never public, v3 is an earlier rebased receipt, and local v4/v5 runs remain outside Git as superseded diagnostics. A receipt cannot include the hash of the commit that first adds itself; the sole post-test overlay is therefore restricted by contract to this receipt, its registry/generator/canonical outputs, this decision, STATUS and the overlay contract itself, and hosted PR checks cover that overlay. |
     ...
     The twelve licensed aggregate identities are rechecked separately while extracting
     the public series; the command fails unless their maximum difference is exactly zero,
     verifies the `4.336808689942018e-19` maximum session-mean difference and compares
     normalized content. The extractor may emit CRLF on Windows, so raw JSON bytes are not
     ...
     $stagedSvg = Join-Path $env:TEMP "cumulative-loss-difference.svg"
     uv run python scripts/render_cumulative_loss_figure.py `
         --run-dir $sourceRun --series-output $stagedSeries --output $stagedSvg
     $recreated = Get-Content $stagedSeries -Raw | ConvertFrom-Json
     if ($recreated.source.maximum_absolute_estimate_difference -ne 0.0 -or
         [math]::Abs($recreated.source.maximum_series_mean_difference -
             4.336808689942018e-19) -gt 1e-30) {
         throw "RP2_PUBLIC_SERIES_IDENTITY_MISMATCH"
     }
     git diff --no-index --ignore-cr-at-eol -- `
         artifacts/rp2_v3/cumulative_loss_session_series_v1.json $stagedSeries
     if ($LASTEXITCODE -ne 0) { throw "RP2_PUBLIC_SERIES_CONTENT_MISMATCH" }
     ```
