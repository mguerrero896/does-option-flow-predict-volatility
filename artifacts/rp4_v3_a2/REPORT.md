...
| [Configuración de ejecución](evaluation_release.json) | `5e3046af023c9f41380307d62537aa4e8fc7514b23d51a95ea1f960ee23fe5bb` |
...
RV30: 195.465 pares finitos, diferencia máxima y media **0**, y 14 pares no finitos coincidentes. No se sustituyeron ni imputaron objetivos. `jump30` se reconstruyó por separado con la máscara de barras observadas del productor RP2; su disponibilidad no excluye filas de la evaluación primaria de QLIKE. [Manifiesto](gamma_manifest.json) · [Cobertura por activo y variable](gamma_coverage.csv).
...
No hay contadores desconocidos ni ventanas vacías en confirmación. Este es el censo del panel de entrada; B4 distingue también el censo efectivamente evaluado y el contraste dentro/fuera. El signo de esos contrastes no se ha calculado en A2. La atribución de las pérdidas históricas de v2 propuesta por el usuario no se convierte aquí en una conclusión causal. [Censo por activo–sesión](empty_window_census.csv) · [Recodificaciones por columna](empty_window_recode_audit.json) · [Manifiesto](empty_window_manifest.json).

## Comandos y verificación

Entorno y comandos completos, versiones, resultados de prueba y hashes: [verificación previa](verification_before_fit.json) y [recibo A2](receipt.json). Se usa el entorno Python 3.12 existente, `uv run --offline --frozen --no-sync`, sin instalar paquetes.
