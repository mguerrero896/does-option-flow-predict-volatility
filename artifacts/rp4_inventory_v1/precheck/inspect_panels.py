...
RUN = Path("D:/MDS650/registered_runs/rp2_v3/rp2-v3-20260901-flow-session-loss-registration")
...
def snapshot() -> dict[str, Any]:
    ...
    for role, step in STEPS.items():
        candidates = {
            name: expected for name, expected in identity["completed_steps"][step]["artifacts"].items()
    ...
    return {
        ...
        "input_manifest_sha256": digest(RUN / "input_manifest.json"),
...
def columns_markdown(document: dict[str, Any]) -> str:
    ...
    for role, panel in document["panels"].items():
        ...
        for key, title in (
            ...
            ("outside_registry_and_keys", "Fuera del registro y las claves; clasificar, no incluir automáticamente"),
            ("inventory_count_reconstruction", "Reconstrucción del conteo solicitado; incluye diagnósticos y no es una lista oficial"),
        ...
        lines += ["### Esquema completo (solo nombres y tipos)", "", "| Columna | Tipo |", "| --- | --- |"]
...
def main() -> None:
    ...
    parser.add_argument("--write", action="store_true", help="Create new inventory files exclusively")
