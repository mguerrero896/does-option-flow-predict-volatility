...
_IN_TREE_FROZEN_REGISTRY = _REPO_ROOT / "data" / "FROZEN_ARTIFACTS.json"
_PACKAGED_FROZEN_REGISTRY = Path(__file__).resolve().parent / "data" / "FROZEN_ARTIFACTS.json"
FROZEN_REGISTRY_PATH = (
    _IN_TREE_FROZEN_REGISTRY
    if _IN_TREE_FROZEN_REGISTRY.is_file()
    else _PACKAGED_FROZEN_REGISTRY
)
...
def frozen_artifact_paths(registry_path: Path | None = None) -> frozenset[str]:
    ...
    path, never an update. The wheel carries the same public registry; its
    absence is a packaging or custody failure and therefore fails closed.
    """
    ...
    if not target.is_file():
        raise FileNotFoundError(f"FROZEN_ARTIFACT_REGISTRY_MISSING:{target}")
