...
def session_minute_expression(column: str = "forecast_origin_utc") -> pl.Expr:
    """Minutes since the 9:30 New York open, as Int64 — the casts are load-bearing.

    ``dt.hour()`` returns Int8 and ``15 * 60`` overflows it silently: measured on
    polars 1.42.1, the 9:30 open came out as −512 and the 15:59 close as −635, a
    deterministic sawtooth that fed HAR's seasonality basis garbage until 2026-08-25.
    ``tests/contract/test_polars_datetime_arithmetic.py`` forbids the uncasted
    pattern repo-wide; this expression is the sanctioned spelling.
    """

    ny = pl.col(column).dt.convert_time_zone("America/New_York")
    return ny.dt.hour().cast(pl.Int64) * 60 + ny.dt.minute().cast(pl.Int64) - 570
    ...
    *,
    ...
    features = (
        ...
        .with_columns(
            ...
            (session_minute_expression() / 390.0).alias("minute_fraction"),
        )
        .with_columns(
...
def fit_log_ols(design: FloatArray, log_target: FloatArray) -> dict[str, FloatArray | float]:
    ...
    return {"coefficients": np.asarray(coefficients, dtype=np.float64), "sigma2": sigma2}
