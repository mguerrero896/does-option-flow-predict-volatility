...
def minimum_detectable_effect_from_long_run_variance(
    ...
    *,
    ...
    """MDE for a session count under the inference contract's long-run variance."""
    ...
    standard_error = math.sqrt(long_run_variance / sessions)
    degrees = sessions - 1
    return float(
...
def minimum_detectable_effect(
    ...
    *,
    ...
    if long_run <= 0.0 and size >= 2:
        long_run = float(np.var(values, ddof=1))
...
def session_contrast(
    ...
    *,
    ...
    return SessionContrast(
        ...
        # Descriptive compatibility with the margin. Confirmatory equivalence additionally
        # requires the registered alpha/multiplicity contract (for example TOST); callers
        # must not promote this convenience flag to that claim.
